 ...
 */
public class AppendFormat implements ExportFormatInterface {
	static final Log LOG = LogFactory.getLog(AppendFormat.class);
	...
	public void init(Configuration conf) {
		// Values will be appended with given column order.
	...
	public String export(Cell kv) throws IOException {
		...
		if (currentKey == null) {
			...
			// Append value with given order
			StringBuilder sb = new StringBuilder();
			sb.append(resultBuf[0]);
			resultBuf[0] = null;
			for (int i = 1; i < resultBuf.length; i++) {
				sb.append(delimiter);
